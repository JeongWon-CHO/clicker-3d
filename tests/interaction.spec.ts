...
test('GLB loads; pointerdown presses, pointerup counts; drag cancels and rotates; count persists', async ({ page }) => {
  const errors: string[] = [];
  ...
  await expect.poll(async () => Number(await canvas.getAttribute('data-press-depth'))).toBeGreaterThan(0.8);
  ...
  await page.mouse.up();
  await expect(count).toHaveText('1');
  await expect.poll(() => page.evaluate(() => localStorage.getItem('clicker:count:v1'))).toBe('1');
  await page.reload();
  await expect(count).toHaveText('1');
...
test('rapid clicks are counted exactly and writes are throttled; pagehide flushes', async ({ page }) => {
  ...
  expect(await page.evaluate(() => localStorage.getItem('clicker:count:v1'))).toBe('15');
...
test('invalid stored count and unavailable storage are handled', async ({ page }) => {
  await page.addInitScript(() => {
...
test('mobile viewport supports tapping without horizontal overflow', async ({ browser }) => {
  ...
  await expect.poll(async () => Number(await page.locator('canvas').getAttribute('data-press-depth'))).toBeGreaterThan(0.8);
  await cdp.send('Input.dispatchTouchEvent', { type: 'touchMove', touchPoints: [{ x: x + 70, y: y + 30, id: 1 }] });
  await cdp.send('Input.dispatchTouchEvent', { type: 'touchEnd', touchPoints: [] });
  await expect(page.getByTestId('count')).toHaveText('1');
  await expect(page.locator('canvas')).toHaveAttribute('data-press-depth', '0.000');
  // Adding a second finger cancels a pending click, even without a drag.
  await cdp.send('Input.dispatchTouchEvent', { type: 'touchStart', touchPoints: [{ x, y, id: 1 }] });
  await cdp.send('Input.dispatchTouchEvent', { type: 'touchStart', touchPoints: [{ x, y, id: 1 }, { x: x + 20, y, id: 2 }] });
  await cdp.send('Input.dispatchTouchEvent', { type: 'touchEnd', touchPoints: [] });
  await expect(page.getByTestId('count')).toHaveText('1');
  await expect(page.locator('canvas')).toHaveAttribute('data-press-depth', '0.000');
  await context.close();
