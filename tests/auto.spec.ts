...
test('custom mode dropdown keyboard and dismissal', async ({ page }) => {
  ...
  await expect(trigger).toHaveText('직접 선택');
...
test('Auto thresholds, collapsed mode picker, manual selection and reset persistence', async ({ page }) => {
  await page.addInitScript(() => {
    if (localStorage.getItem('clicker:count:v1') === null) localStorage.setItem('clicker:count:v1', '99');
  });
  await page.goto('/');
  ...
  await expect.poll(() => page.evaluate(() => localStorage.getItem('clicker:count:v1'))).toBe('100');
  ...
  await button('화난 종근이').click();
  await click();
  await expect(controls).toHaveAttribute('data-face', 'angry');
  await page.locator('.count-panel').getByRole('button', { name: '클릭 수 초기화' }).click();
  ...
  expect(await page.evaluate(() => localStorage.getItem('clicker:count:v1'))).toBe('0');
  await page.reload();
  await expect(page.getByTestId('count')).toHaveText('0');
  await expect(page.getByRole('combobox', { name: '표정 모드' })).toHaveText('Auto');
});
