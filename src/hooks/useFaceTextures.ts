...
import { faceVariants, initialFaceStatuses, type FaceStatuses, type FaceVariant } from "../faceVariants";

export function useFaceTextures(template: Texture, onStatus: (status: FaceStatuses) => void) {
  const { gl, invalidate } = useThree();
  const [textures, setTextures] = useState<Partial<Record<FaceVariant, Texture>>>({});
  ...
  useEffect(() => {
    ...
    for (const variant of faceVariants) {
      ...
      loader.load(variant.textureUrl, texture => {
